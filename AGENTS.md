...
- Migrations: SQL migrations в `api/migrations.js`, runner `backend/scripts/migrate-vercel-api.mjs`.
...
- `backend/scripts/migrate-vercel-api.mjs` - migration runner для `pnpm run db:migrate` и production build.
- `frontend` - основной React/Vite frontend. Страницы в `src/pages`, UI в `src/components`, feature logic в `src/features`, helpers в `src/lib`.
- `docs/mockup-sandbox` - старый sandbox-артефакт, не является runtime mode и не должен влиять на production-путь.
...
- `backend/scripts` - helper scripts, включая package-manager guard, post-merge hook и migration runner.
...
- `docs/replit.md` - текущая архитектурная заметка по Vercel-only workspace.
...
Когда функциональность меняется, в рамках той же задачи обновлять связанные документы: `README.md`, `docs/replit.md`, `openapi.yaml`, `.env.example`, `vercel.json`, `api/migrations.js` и этот `AGENTS.md`, если применимо. Исторические документы из `../` обновлять только если задача явно продолжает тот пакет анализа или меняет его выводы.
...
- `docs/replit.md` обновлять при изменениях architecture, stack, command, route или database.
...
- Пользователь предпочитает умеренную структурную реорганизацию, если она улучшает читаемость проекта без риска сломать Vercel platform contract.
...
- Root static snapshot и GitHub Pages workflow удалены; Vercel output остаётся `frontend/dist/public`.
...
- После умеренной реорганизации основной frontend живёт в `frontend`, архивный mockup sandbox - в `docs/mockup-sandbox`, helper scripts - в `backend/scripts`; Vercel `api/` остаётся в корне как platform contract.
